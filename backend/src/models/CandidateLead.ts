...
// Indexes for high-performance querying and admin filters
CandidateLeadSchema.index({ phone: 1 });
CandidateLeadSchema.index({ status: 1, createdAt: -1 });
CandidateLeadSchema.index({ isSolved: 1 });
CandidateLeadSchema.index({ industry: 1, preferredLocation: 1 });

export const CandidateLead = mongoose.model<ICandidateLead>('CandidateLead', CandidateLeadSchema);
